...
pd = colData(rse_gene)
pd$Dx = factor(pd$Dx, levels = c("Control", "Bipolar"))
...
pgcEqtlsFinal = read.csv("raggr_881_snps_dlpfc_eqtls_fdr01.csv")
...
# statOut$feature = rownames(statOut)

# ### put in order
# statOut = statOut[match(eqtl$Feature,statOut$feature),]

# ### add dir and pval from case-control DE
# statOut$bipolar_dir = ifelse(statOut$logFC_dlpfc > 0, "UP", "DOWN")
# eqtl$bipolar_dir = statOut$bipolar_dir
# eqtl$bipolar_pval = statOut$P.Value_dlpfc

eqtl = eqtl[order(eqtl$pgcFinalRank, eqtl$pvalue),]

write.csv(eqtl, file="pgc_conditional_by_leadVar_dlpfc.csv")
