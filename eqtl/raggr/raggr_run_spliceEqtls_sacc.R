...
#####################################
####### read in splice files ########
#####################################

spliceNorm = readRDS("/users/schadinh/lieber/leafcutterRDS/leafcutter_sACC_annotated.rds")
...
save(genePCs, exonPCs, jxnPCs, txPCs, 
	file="rdas/pcs_4features_sacc.rda")
# load("rdas/pcs_4features_sacc.rda")
...
save(meGene, meExon, meJxn, meTx,
	file="matrixEqtl_output_sacc_raggr_4features.rda")
...
allEqtl$gencodeTx = CharacterList(c(as.list(rowRanges(rse_gene)$gencodeTx[match(geneEqtl$gene, 
	...
	as.list(txEqtl$gene)))
save(allEqtl, file="mergedEqtl_output_sacc_raggr_4features.rda",compress=TRUE)
